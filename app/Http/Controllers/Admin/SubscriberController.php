...
class SubscriberController extends AdminController
{
    ...
    public function index(string $lang, AdminUserRequest $request)
    {
        ...
        return view('admin.modules.subscriber.index', [
            'subscribers' => Subscriber::query()->paginate(10),
        ]);
    }
    ...
    {
        if (!Subscriber::query()->where('id',$id)->delete()) {
            return redirect(route('subscriberIndex', $locale))->with('danger', trans('admin.subscriber_not_delete'));
        }
        return redirect(route('subscriberIndex', $locale))->with('success', trans('admin.subscriber_success_delete'));
