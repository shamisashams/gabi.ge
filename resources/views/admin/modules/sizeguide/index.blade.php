            ...
            <div class="col s12">
                <div class="card">
                    ...
                    <div class="card-content">
                        <a href="{{route('sizeguideCreate',app()->getLocale())}}"
                           class="mb-2 btn waves-effect waves-light green darken-1">{{trans('admin.create_shipping')}}</a>
                        <div style="overflow: auto">
                            {!! Form::open(['url' => route('shipping.index',app()->getLocale()),'method' =>'get']) !!}
                            <ul>
                                <li>
                                    @if ($errors->has('id'))
                            ...
                            <table class="striped">
                                ...
                                @if($data)
                                    @foreach($data as $item)
                                        <tr>
                                            <td>{{$item->id}}</td>
                                            <td>{{isset($item->availableLanguage[0])?  $item->availableLanguage[0]->age : ''}}</td>
                                            <td>
                                                <a href="{{route('sizeguideEdit',[app()->getLocale(),$item->id])}}"><i
                                                        class="material-icons">edit</i></a>
                                                {{-- <a href="{{route('shipping.destroy',[app()->getLocale(),$item->id])}}"><i
                                                        class="material-icons">remove_red_eye</i></a> --}}


                                                        {!! Form::open(['url' => route('sizeguide.destroy',[app()->getLocale(),$item->id]),'style'=>'display:inline-block']) !!}
